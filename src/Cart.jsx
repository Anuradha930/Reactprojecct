...
function Cart() {
  ...
  const handleRemove = (id, name) => {
    ...
    safeToast(`remove-${id}`, "error", `❌ ${name} removed from cart`, { position: "top-center" });
  };

  const handleIncrement = (id, name) => {
    dispatch(incrementQuantity(id));
    safeToast(`inc-${id}`, "info", `➕ Increased ${name}`, { position: "top-center" });
  };

  const handleDecrement = (id, name) => {
    dispatch(decrementQuantity(id));
    safeToast(`dec-${id}`, "info", `➖ Decreased ${name}`, { position: "top-center" });
  };

  // ✅ Apply button discount
  const applyDiscount = (percent) => {
    setButtonDiscount(percent);
    safeToast(
      `discount-${percent}`,
      "success",
      `🎉 ${percent}% discount applied! You saved ₹${((totalPrice * percent) / 100).toFixed(2)}`,
      { autoClose: 1200 }
    );
  };
  ...
  const applyCoupon = () => {
    ...
    if (result.isValid) {
      ...
      safeToast("coupon-invalid", "error", "❌ Invalid coupon code", { autoClose: 2000 });
    }
  };
  ...
  const handleCheckout = () => {
    ...
    const templateParams = {
      ...
      cost: {
        ...
        discount: (buttonDiscountAmount + couponDiscountAmount).toFixed(2),
    ...
    Swal.fire({
      ...
      balloonsUp(5000);
      safeToast("sending-email", "info", "⏳ Sending order confirmation...", { autoClose: 1500 });
    ...
    <>
      ...
      {cartItems.length === 0 ? (
        <div className="empty-cart">
          <img src="/Images/emptyCard.avif" alt="Empty Cart" />
        ...
        <div className="cart-container">
            ...
            {cartItems.map((item) => (
              <div key={item.id} className="cart-item">
                ...
                <div className="cart-item-actions">
                  <button className="btn-dec" onClick={() => handleDecrement(item.id, item.name)}>-</button>
                  <span>{item.quantity}</span>
                  <button className="btn-inc" onClick={() => handleIncrement(item.id, item.name)}>+</button>
                  <button className="btn-remove" onClick={() => handleRemove(item.id, item.name)}>Remove</button>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="cart-sidebar">
            <div className="price-details">
              ...
              {buttonDiscount > 0 && (
                <p className="text-success">
                  Button Discount: {buttonDiscount}% (-₹{buttonDiscountAmount.toFixed(2)})
                </p>
              )}
              {couponResult.isValid && (
                <p className="text-success">
                  Coupon Discount: {couponResult.discountPercent}% (-₹{couponDiscountAmount.toFixed(2)})
                </p>
              )}
            ...
            <div className="discount-buttons">
              <button className="btn-discount" onClick={() => applyDiscount(10)}>10% Discount</button>
              <button className="btn-discount" onClick={() => applyDiscount(20)}>20% Discount</button>
              <button className="btn-discount" onClick={() => applyDiscount(30)}>30% Discount</button>
              <button className="btn-reset" onClick={resetDiscounts}>Reset</button>
            </div>

            {/* Payment */}
            <div className="payment-method">
              <h5>Select Payment Method</h5>
              <button className="btn-payment" onClick={() => setPaymentMethod("qr")}>QR Code</button>
              <button className="btn-payment" onClick={() => setPaymentMethod("card")}>Card</button>
            </div>
            ...
            <button className="checkout-btn" onClick={handleCheckout}>Purchase & Checkout</button>
            ...
            {paymentMethod === "qr" && (
              <div className="qr-section">
                ...
                <QRCode
                  value={`upi://pay?pa=9391271915-3@ybl&pn=RoyalFood&am=${finalPrice.toFixed(2)}&cu=INR`}
                  size={150}
                />
              </div>
            )}
